...
<body>
	...
	<section>
		<div class="mainimg">
			...
			<div class="p_message">
				<span><a href="../main" style="color: blue;">HOME</a>>커뮤니티>자유게시판</span>
			</div>
		</div>
	</section>
				...
				<tr>
					...
					<c:if test="${session_ucno==2}">
						...
						<td>
							<p>
								<a
									href="/board/board_comm?bc_no=${map.nDto.bc_no}&bt_no=${bTDto.boardDto.bt_no}&page=${map.nDto.page}&category=${map.nDto.category}&searchword=${map.nDto.searchword}">${bTDto.boardDto.bt_title}</a>
							</p>
						</td>
					</c:if>
					<td class="on">
	...
	<section id="contentArea">
		...
		<div class="board_btn">
			<div>
				<button type="button" id="user_write_btn"
					onclick="location.href='board_write?bc_no=${map.nDto.bc_no}'">글쓰기</button>
			</div>
		</div>
	...
	<!--}contentArea-->
	<ul class="page-num" id="page_num_num">

		<a href="board_free?bc_no=1&page=1&bc_no=1&category=${map.nDto.category}&searchword=${map.nDto.searchword}"><li
			class="first"></li></a>
		...
		<c:if test="${map.nDto.page>1}">
			<a
				href="board_free?bc_no=1&page=${map.nDto.page-1}&category=${map.nDto.category}&searchword=${map.nDto.searchword}"><img
		...
		<c:forEach var="nowpage" begin="${map.nDto.startpage}"
			...
			<li class="num"><c:if test="${map.nDto.page!=nowpage}">
					<a
						href="board_free?bc_no=1&page=${nowpage}&category=${map.nDto.category}&searchword=${map.nDto.searchword}"><div>${nowpage}</div></a>
		...
		<a href="board_free?bc_no=1&page=${map.nDto.maxpage}&category=${map.nDto.category}&searchword=${map.nDto.searchword}">
		<img src="../images/page_right1_new.gif" alt="다음"></li></a>
	</ul>
			...
			<div class="s_search">
				<div class="s_down">
					<div id="search_box_type">
						<input type="hidden" name="bc_no" value="1"> <select
							name="category" id="category" style="border: none;">
