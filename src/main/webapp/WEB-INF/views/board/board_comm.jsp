	...
	<section id="contentArea">
		...
		<div id="content_view">
			<div class="content_view_top">
				<div id="content_view_title">
					<span>제목 :&nbsp&nbsp&nbsp </span>${map.bDto.boardDto.bt_title}</div>
				<!-- 날짜, hit 값 들어감 -->
				<div id="content_view_date">

					<span style="color: green;"> <c:if
							test="${map.bDto.memberDto.uc_no==1}">
                    [유저]</c:if> <c:if test="${map.bDto.memberDto.uc_no==2}">[농장주]</c:if>


						${map.bDto.memberDto.u_nickname}&nbsp&nbsp&nbsp&nbsp
					</span> <span style="padding-right: 20px; border-right: 1px solid #666;">${map.bDto.boardDto.bt_date }</span><span
						style="padding-left: 20px;">조회수 : </span><span>${map.bDto.boardDto.bt_hit}</span>
				</div>
			</div>
			...
			<div id="content_view_mid">
				<table>
					<c:if test="${map.bDto.boardDto.bt_upload != null }">
						<tr>
							<td colspan="4" class="article"><img
								src="../upload/${map.bDto.boardDto.bt_upload}"
								style="max-height: 1000px; max-width: 600px; margin: 0 auto;">
							</td>
						</tr>
					</c:if>

					<p  style="white-space:pre-wrap;">${map.bDto.boardDto.bt_content}</p>

				</table>
			</div>

			<form action="./content_btn_report" name="report" id="report"
				method="get">
				<div class="board_btn">
					<input type="hidden" name="bt_no"
						value="${map.bDto.boardDto.bt_no}"> <input type="hidden"
						name="bc_no" value="${map.bDto.boardDto.bc_no}">
                    ...
                    [유저]</c:if>
						...
						<div id="coment_view_comm_modify">
							...
							<c:if test="${comm.memberDto.u_no==session_uno}">
								<button type="button"
									onclick="commentModifyBtn(${comm.boardDto.bcm_no},'${comm.boardDto.bcm_nickname}','${comm.boardDto.bcm_date}','${comm.boardDto.bcm_content}',${comm.boardDto.bt_no},${comm.memberDto.uc_no})">수정</button>
								<button type="button"
									onclick='commentDeleteBtn(${comm.boardDto.bcm_no})'>삭제</button>
							</c:if>
			...
			<div id="content_view_bottom1">
				...
				<div id="content_view_bottom2">
					...
					<div id="content_view_next">
						<a
							href="./board_comm?bc_no=${map.bDto.boardDto.bc_no}&bt_no=${map.preDto.boardDto.bt_no}&page=${nDto.page}&category=${nDto.category}&searchword=${nDto.searchword}">${map.preDto.boardDto.bt_title}</a>
					</div>
				</div>
				<label>이전글</label>
				<div id="content_view_prev">
					<a
						href="./board_comm?bc_no=${map.bDto.boardDto.bc_no}&bt_no=${map.nextDto.boardDto.bt_no}&page=${nDto.page}&category=${nDto.category}&searchword=${nDto.searchword}">${map.nextDto.boardDto.bt_title}</a>
				</div>
			</div>

			<div class="board_btn">
				<button type="button" class="content_btn"
					onclick="location.href='board_comm_back?bc_no=${map.bDto.boardDto.bc_no}'">목록</button>

				<c:if test="${session_uno==map.bDto.memberDto.u_no}">
					<form action="./board_user_Delete" id="delete_board" method="post">
						<input type="hidden" name="bc_no"
							value="${map.bDto.boardDto.bc_no}"> <input type="hidden"
							name="bt_no" value="${map.bDto.boardDto.bt_no}">
						<button type="button" class="content_btn"
							onclick="delete_board_btn()">삭제</button>
					</form>
				</c:if>
			</div>
		</div>
