...
<body>
    <section class="mlist_body">
        ...
        <table>
            ...
            <tr>
                <td>${farmDto.fr_no }</td>
                <td><a class="farmname" href="../farm/farm_view?fr_no=${farmDto.fr_no}&page=${map.pDto.page}&searchword=${map.pDto.searchword}">${farmDto.fr_name }</a></td>
                <td>${farmDto.fr_addr }</td>
                <td><fmt:formatNumber value="${farmDto.fr_price}" pattern="#,###"/>원</td>
        ...
        <!-- 페이징 -->
		<ul class="page-num">
			<a href="man_farm_list?page=1&searchword=${map.pDto.searchword}"><li class="first"></li></a>
			
			<c:if test="${map.pDto.page>1 }">
				<a href="man_farm_list?page=${map.pDto.page-1}&searchword=${map.pDto.searchword}"><li class="prev"></li></a>
			</c:if>
			
			<c:forEach var="nowpage" begin="${map.pDto.startpage }" end="${map.pDto.endpage }">
				<li class="num">
					<c:if test="${map.pDto.page!=nowpage}">
					<a href="man_farm_list?page=${nowpage}&searchword=${map.pDto.searchword}"><div>${nowpage}</div></a>
					</c:if>
			...
			<c:if test="${map.pDto.page<map.pDto.maxpage }">
				<a href="man_farm_list?page=${map.pDto.page+1}&searchword=${map.pDto.searchword}"><li class="next"></li></a>
			</c:if>
			
			<a href="man_farm_list?page=${map.pDto.maxpage}&searchword=${map.pDto.searchword}"><li class="last"></li></a>
		</ul>
		
        <div class="mfsearchbox">
        <form action="man_farm_list">
