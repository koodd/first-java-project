...
public class FarmServiceImpl implements FarmService {
	...
	// 농장 예약 결제
	@Override
	public void farmDoReser(int py_price, int u_no, int fr_no, String frs_startdate, String frs_account) {
		farmMapper.insertPayFarm(py_price, u_no); // 결제 테이블
		farmMapper.insertFarmReser(u_no, fr_no, frs_startdate, frs_account); // 구매 내역 테이블
	}
	...
	public Map<String, Object> farmlist(NumberDto nDto) {
		Map<String, Object> map = new HashMap<String, Object>();
		// 페이징 메소드 호출
		NumberDto pDto = paging(nDto, 4);
		// 게시글 가져오기
		List<FarmDto> list = farmMapper.selectList(pDto.getStartrow(), pDto.getEndrow(), pDto.getSearchword(),
				pDto.getCategory(), pDto.getRegion());
	...
	public NumberDto paging(NumberDto pageDto, int lim) {
		...
		NumberDto pDto = new NumberDto(page, limit, numLimit, listCount, maxPage, startPage, endPage, startrow, endrow,
				category, searchword, region);
		return pDto;
	}
	...
	// 농장 등록 여부 확인
	@Override
	public int farmRegiCheck(int uno) {
		int result = farmMapper.farmCheckRegi(uno);
		return result;
	}
	...
	// 인기 있는 농장 TOP7
	@Override
	public Map<String, Object> farmToplist() {
		Map<String, Object> map = new HashMap<String, Object>();
		List<FarmDto> list = farmMapper.farmTopList();
		
		List<FarmDto> fList = new ArrayList<FarmDto>();
		
		for(int i=0; i<list.size(); i++) { 
			 fList.add(farmMapper.man_farm_fegi_view(list.get(i).getFr_no()));
		}
		map.put("fList", fList);
		return map;
	}
