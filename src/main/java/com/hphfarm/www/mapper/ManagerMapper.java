...
// =========================== 최건호 ===========================
	...
	// 관리자페이지 : 체험 목록 확인하기 >> 체험 게시글 전체 가져오기
		...
		List<ExpDto> man_exp_bookSelect(int startrow, int endrow, String category, String searchword);
		
		// 관리자페이지 유저가 예약한 현황 보기 하단넘버링 - 총 게시글 수
		int man_exp_bookSelectCount(String category, String searchword);
	...
	ManagerDto mainData();
	Integer payTotal();
	List<ManagerDto> monthStatistics();
	ManagerDto sumPay();
	...
	// 1:1 문의 내역 삭제
	void updateInquiry(ManagerDto mDto);
}
