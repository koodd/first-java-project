    ...
    <script type="text/javascript">
    	function deleteBtn(bn_no){
    		if(confirm("해당 게시글을 삭제 하시겠습니까?")){
    			location.href="./delete_board_notice?bn_no="+bn_no;
    		}else{
    			return false;
         ...
         <tr>
	      ...
	      <tr>
            ...
            <div id="content_view_bottom2">
             ...
             <a href="./man_public_board_view?bn_no=${map.preDto.bn_no}&page=${nDto.page}&category=${nDto.category}&searchword=${nDto.searchword}">${map.preDto.bn_title}</a>        
                  </div>
            </div>
                <label>이전글</label>
                <div id="content_view_prev">
                    <a href="./man_public_board_view?bn_no=${map.nextDto.bn_no}&page=${nDto.page}&category=${nDto.category}&searchword=${nDto.searchword}">${map.nextDto.bn_title}</a>
                </div>
            </div>
       
            <div class="board_btn">           	    
                <button type="submit" class="content_btn managerbuttoncss" onclick="location.href='man_public_board_list?page=${nDto.page}&category=${nDto.category}&searchword=${nDto.searchword}'">목록</button>
            	<button type="button" class="content_btn2 managerbuttoncss" onclick="deleteBtn(${map.bDto.bn_no})" >삭제</button>
            	<button type="submit" class="content_btn2 managerbuttoncss" onclick="location.href='man_public_modify_view?bn_no=${map.bDto.bn_no}'">수정</button>
               </div>
