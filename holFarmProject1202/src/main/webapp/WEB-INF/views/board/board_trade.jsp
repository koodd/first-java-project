...
<body>
    ...
    <section>
                ...
                <table class="tabMenu">
                    ...
                    <tbody>
                       <tr>
							...
							<%-- 			<c:if test="${session_ucno==null}">
								<td ><p><a href="../main" style="border-left: none;">메인 페이지로</a></p></td>
										</c:if> --%>
      ...
      <a href="board_trade?page=1&category=${map.nDto.category}&searchword=${map.nDto.searchword}"><li class="first"></li></a>
      <c:if test="${map.nDto.page<=1}">
         <li class="prev"></li>
      </c:if>
      <c:if test="${map.nDto.page>1}">
         <a href="board_trade?page=${map.nDto.page-1}&category=${map.nDto.category}&searchword=${map.nDto.searchword}"><img src="../images/page_right3_new.gif" alt="이전"></a>
      </c:if>
      <c:forEach var="nowpage" begin="${map.nDto.startpage}" end="${map.nDto.endpage}">
         <li class="num">
           <c:if test="${map.nDto.page!=nowpage}">
             <a href="board_trade?page=${nowpage}&category=${map.nDto.category}&searchword=${map.nDto.searchword}"><div>${nowpage}</div></a>
           </c:if>
           <c:if test="${map.nDto.page==nowpage}">
             <div>${nowpage}</div>
           </c:if>
      ...
      <c:if test="${map.nDto.page<map.nDto.maxpage}">
        <a href="board_trade?page=${map.nDto.page+1}&category=${map.nDto.category}&searchword=${map.nDto.searchword}"><img src="../images/page_right1_new.gif" alt="다음"></li></a>
      </c:if>
      <c:if test="${map.nDto.page>=map.nDto.maxpage}">
        <li class="next"></li>
      </c:if>
       <a href="board_trade?page=${map.nDto.maxpage}&category=${map.nDto.category}&searchword=${map.nDto.searchword}"></a>
   		 </ul>
            ...
            <div class="s_search">
                <div class="s_down">
                    <div id="search_box_type">
                      <input type="hidden" name="bc_no" value="4"> 
