					...
					<c:forEach items="${map.list}" var="bTDto">
						...
						<div class="b_tr">
							...
							<c:forEach begin="1" end="${bTDto.boardDto.bt_indent}">
								...
								<a href="./my_board_view?bt_no=${bTDto.boardDto.bt_no}&page=${map.nDto.page}&category=${map.nDto.category}&searchword=${map.nDto.searchword}">${bTDto.boardDto.bt_title}</a>								
							</div>
...
<ul class="page-num">
      ...
      <c:if test="${map.nDto.page>1}">
         <a href="my_board_list?page=${map.nDto.page-1}&category=${map.nDto.category}&searchword=${map.nDto.searchword}"><img src="../images/page_right3_new.gif" alt="이전"></a>
      </c:if>
      ...
      <c:if test="${map.nDto.page<map.nDto.maxpage}">
        <a href="my_board_list?page=${map.nDto.page+1}&category=${map.nDto.category}&searchword=${map.nDto.searchword}"><img src="../images/page_right1_new.gif" alt="다음"></li></a>
      </c:if>
      <c:if test="${map.nDto.page>=map.nDto.maxpage}">
        <li class="next"></li>
      </c:if>
       <a href="my_board_list?page=${map.nDto.maxpage}&category=${map.nDto.category}&searchword=${map.nDto.searchword}"></a>
   		 </ul>
